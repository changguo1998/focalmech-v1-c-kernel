...
function read_c_result(io::IO)
    ...
    nmts = read(io, Int64)
    nsrc = read(io, Int64)
    misfit_pl2 = zeros(Float32, nmts, nsrc, ntrs)
    misfit_pshift = zeros(Int64, nmts, nsrc, ntrs)
    misfit_sl2 = zeros(Float32, nmts, nsrc, ntrs)
    misfit_sshift = zeros(Int64, nmts, nsrc, ntrs)
    misfit_pol = zeros(Float32, nmts, nsrc, ntrs)
    misfit_psr = zeros(Float32, nmts, nsrc, ntrs)
    read!(io, misfit_pl2)
