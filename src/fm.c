...
int main() {
    ...
    printf("Load data\n");
    read_input("input.bin", &n_obs_traces, &n_mt, &n_srcloc, &traces, &mts,
        &gf_database);
    ...
    printf("Output\n");
    write_result("output.bin", n_obs_traces, n_mt, n_srcloc, misfit_pl2,
        misfit_pshift, misfit_sl2, misfit_sshift, misfit_polarity,
        misfit_psr);
    ...
    free(traces);
    free(mts);
    for(int i = 0; i < n_obs_traces * n_srcloc; i++) free_gf_trace(&(gf_database[i]));
    free(gf_database);
    free(misfit_pl2);
    free(misfit_pshift);
    free(misfit_sl2);
