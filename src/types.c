...
#define readint(var) fread(&(var), sizeof(int64_t), 1, fp)
#define writeint(var) fwrite(&(var), sizeof(int64_t), 1, fp)
...
void read_trace(Trace* tr, FILE* fp) {
    ...
    readint(tr->swin_l);
#ifdef DEBUG
    printf("(read_trace)tr: %p, n: %lld, ref: %lld, pwin_i: %lld, pwin_l: %lld, swin_i: %lld, swin_l: %lld, %p, ",
...
void read_gf_trace(GFtrace* gf, FILE* fp) {
    ...
    readint(gf->s_i);
#ifdef DEBUG
    printf("(read_gf_trace) npts: %lld, pi: %lld, si: %lld\n", gf->n_samples, gf->p_i, gf->s_i);
#endif
